"""
mmfinereason_sft_reasoning_ver2.py

Generates new reasoning + response using the full input context from metadata:
  question + image + qwen3vl_235b_thinking_response + answer → new reasoning + new response

All other options (--subsets, --samples, --retry-index, generation config, etc.)
are identical to mmfinereason_sft_reasoning.py.
"""
...
def build_user_content(template, question, options, base64_image,
    ...
    # Order: image → question+options → reasoning → response → template → question
    content = []
...
def process_sample(client, model, template, sample, image_dir, gen_config):
    ...
    qwen_thinking = sample.get("qwen3vl_235b_thinking_response", "") or ""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MMFineReason Reasoning v2 — question + image + qwen_thinking + answer as input"
    )
    ...
    parser.add_argument(
        ...
        help="Output directory (default: data/reasoning/reasoning_v2_YYYYMMDD_HHMMSS/)",
    )
    parser.add_argument(
    ...
    if args.output_dir is None:
        ...
        args.output_dir = os.path.join(
            dataset_root, "data", "reasoning_with_ref", f"reasoning_v2_{timestamp}"
        )

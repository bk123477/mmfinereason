...
def build_user_content(template, question, options, base64_image):
    ...
    prompt_text = (
        ...
        f"### Input Question:\n{question}"
        f"{options_str}\n\n"
        f"Please follow the structured reasoning phases defined above and provide the final answer."
    )
...
def process_sample(client, model, template, sample, image_dir, gen_config):
    ...
    if isinstance(question, list):
        # conversations format: [{from: human, value: ...}, ...]
    ...
    content = build_user_content(template, question, options, base64_image)
    ...
    try:
        ...
        # OpenRouter returns chain-of-thought in message.reasoning when include_reasoning=True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MMFineReason Reasoning Distillation via OpenRouter"
    )
    parser.add_argument("--api-key", required=True, help="OpenRouter API Key")
    parser.add_argument("--model", required=True, help="OpenRouter model ID")
    parser.add_argument(
        "--template-path", default=TEMPLATE_DEFAULT, help="Path to reasoning_distillation.md"
    )
    parser.add_argument(
        "--data-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Directory to save reasoning output JSON files (default: data/reasoning/reasoning_YYYYMMDD_HHMMSS/)",
    )
    parser.add_argument(
        "--subsets",
        nargs="*",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of samples to process per subset (default: all available)",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument("--n",                  type=int,   default=1,     help="Number of generations per sample (default: 1)")
    parser.add_argument("--max-tokens",         type=int,   default=81920, help="Max output tokens (default: 81920)")
    args = parser.parse_args()

    # Build timestamped output dir if not explicitly provided
    if args.output_dir is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        args.output_dir = os.path.join(dataset_root, "data", "reasoning", f"reasoning_{timestamp}")

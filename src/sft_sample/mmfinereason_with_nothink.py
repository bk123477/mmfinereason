...
Identical to mmfinereason_sft_reasoning_ver2.py, with one difference:
  The <think>...</think> block is stripped from qwen3vl_235b_thinking_response
  before it is used as input context. Only the text AFTER </think> is kept.

Input context:
  question + image + qwen_thinking (think block removed) + answer → new reasoning + new response

Output saved under: data/reasoning_nothink/reasoning_nothink_YYYYMMDD_HHMMSS/
"""
...
def strip_think_block(text):
    """Remove <think>...</think> (including tags) and return only the text after it."""
    if not text:
        return text
...
def process_sample(client, model, template, sample, image_dir, gen_config):
    ...
    # Strip <think>...</think> from the reference reasoning
    raw_thinking  = sample.get("qwen3vl_235b_thinking_response", "") or ""
    qwen_thinking = strip_think_block(raw_thinking)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Output directory (default: data/reasoning_nothink/reasoning_nothink_YYYYMMDD_HHMMSS/)",
    )
    parser.add_argument(
    ...
    if args.output_dir is None:
        ...
        args.output_dir = os.path.join(
            dataset_root, "data", "reasoning_nothink", f"reasoning_nothink_{timestamp}"
        )

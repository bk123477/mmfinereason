...
Step 1 (Detection):
  LLM + rule-based checks return {c1, c2, c3, c4} booleans.

Step 2 (Rewrite — ALL samples):
  Path A — Clean (no issues detected):
    Light rewrite using post_filter_rewrite_clean_prompt.md
    → 6-phase structure + remove any slipped reference contamination.
    No re-detection loop (single pass).

  Path B — Flagged (issues detected):
    Full rewrite using post_filter_rewrite_prompt.md
    → LLM selects one of 10 diverse reasoning workflows, fixes all C1~C4.
    → Re-detect after each rewrite. Repeat up to --max-iterations times.

Multiple samples are processed concurrently (--workers controls parallelism).

Quality issues:
  C1: Reference contamination in reasoning
  C2: Reasoning structure violation (no valid workflow)
  C3: Workflow/phase labels leaked into response
...
# Hard string patterns that always indicate C1/C4 contamination
C1_HARD_PATTERNS = [
    ...
    # Self-aware hedging — model knows it has reference but tries to suppress it
    "which I must not",
    "I must not mention",
    "I must not copy",
    "I must not directly",
...
# Retroactive C2: if the first Phase label appears after this fraction of the text
...
def build_clean_message(question: str, options, reasoning: str, response: str, answer: str) -> str:
    """Message for the clean rewrite path (no issues detected — preservation-first pass)."""
...
def rule_detect_c1(text: str) -> bool:
    """Return True if any hard contamination pattern is found in text (case-insensitive check for some)."""
    if not text:
        return False
...
def rule_detect_c2(reasoning: str) -> bool:
    ...
    if len(content_sections) >= 4:
        ...
        if first_bracket_pos != -1 and first_bracket_pos / len(reasoning) <= C2_RETROACTIVE_THRESHOLD:
            return False  # valid bracket workflow structure

    return True  # no valid structure found
...
async def detect_issues_async(
    ...
    c4_rule = rule_detect_c1(response)  # same patterns apply to response
    ...
    try:
        ...
        # Log if rule-based caught something the LLM missed
...
def parse_rewrite_output(raw: str) -> dict | None:
    ...
    wf_match = re.search(r"<WORKFLOW>(.*?)</WORKFLOW>",    raw, re.DOTALL)
    if not r_match and not p_match:
        return None
    reasoning_text = r_match.group(1).strip() if r_match else None
    # Workflow name comes from the separate <WORKFLOW> tag (never inside reasoning)
    workflow = wf_match.group(1).strip() if wf_match else None
    return {
...
async def process_sample_async(
    ...
    if not any(issues.values()):
        user_msg = build_clean_message(question, options, reasoning, response, answer)
        raw = await call_api_async(
            client, semaphore, model,
        ...
        async def process_one(sample: dict):
            ...
            async with lock:
                ...
                if result is None:
                    ...
                    if wf == "clean-preserve":
                        stats["clean"] += 1
                        print(
                            f"  [{completed:>3}/{total_pending}] {subset_name}_{idx}"
                            f"  → clean-preserve {tag}"
                        )
                    else:
                        stats["rewritten"] += 1
                        short_wf = wf.split("(")[0].strip() if wf else "?"
                        print(
                            f"  [{completed:>3}/{total_pending}] {subset_name}_{idx}"
                            f"  → [{flags}] {short_wf} {tag} ({iters} iter(s))"
                        )

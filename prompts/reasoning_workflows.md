...
Each workflow is optimized for a specific problem type. When rewriting a reasoning trace,
analyze the question text and the existing reasoning draft to infer the problem type,
then select the single most appropriate workflow.
(You do not have access to the image — use the reasoning draft's descriptions of visual content
to understand what was in the image.)

**Section header format:** Write each section header on its own line in square brackets.
Immediately follow the header with the reasoning content for that section.

Example:
```
[Section Name]
Reasoning content for this section goes here...
...
```
[Scene Assessment]
[Evidence Scan — Pass 1]
[Evidence Scan — Pass 2]
